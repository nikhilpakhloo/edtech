export const APP_STRINGS = {
  ...
  detail: {
    ...
    shareAction: 'Share',
    trendingMetric: 'Trending',
    episodeLabel: (count: number) => `${count} episodes`,
    episodeBullet: (count: number) => `${count} focused episodes with clear checkpoints`,
    languageBullet: (languages: string) => `Available in ${languages}`,
    outcomeForGenre: (genre: string) => `Build practical confidence in ${genre}`,
    selfPacedBullet: 'Self-paced playback with resume support',
    sessionBullet: (runtime: string) => `Designed as a ${runtime} learning session`,
    metrics: {
      level: 'Level',
      progress: 'Progress',
      runtime: 'Runtime',
